...
const baseUrl = process.env.BOOK_PICKER_URL?.trim() || 'http://127.0.0.1:43941';
...
try {
  ...
  // Book chip at cocos (0,348) → design top-left (720, 405-348).
  const interactT0 = Date.now();
  const chipPt = designPoint(vp, 720, 405 - 348);
  ...
  // Drag list upward → windowStart should increase.
  const panelTopDesign = (405 - 348) + 43;
