...
const baseUrl = process.env.SETTLEMENT_BASE_URL?.trim() || 'http://127.0.0.1:43891';
...
async function completeQuestion(page, testCase) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    await press(page, testCase, 320, 430);
    try {
  ...
  if (!answered) {
    const state = await page.evaluate(() => ({
      answerReady: document.body.dataset.answerReady,
      gameScore: document.body.dataset.gameScore,
    ...
    throw new Error(`${testCase.scene} did not accept answer: ${JSON.stringify(state)}`);
  }
  await page.waitForFunction(({ questionId, scene }) => (
    document.body.dataset.stageResult === scene
      || (
        document.body.dataset.answerReady === 'true'
        && document.body.dataset.questionId !== questionId
      )
  ), { questionId: previousQuestionId, scene: testCase.scene }, { timeout: 7000 });
  return await page.getAttribute('body', 'data-stage-result') === testCase.scene;
}
...
async function inspectResult(page, testCase, screenshot) {
  ...
  const edge = edgeMetrics(PNG.sync.read(screenshot));
  ...
  if (!Number.isFinite(runtime.resultLayout.artworkScale)
    || Math.abs(runtime.resultLayout.artworkScale - 1) > 0.001
    || !Number.isFinite(runtime.resultLayout.artworkOffsetY)
    ...
    throw new Error(
      `${testCase.scene} result artwork or mapped stretch-X positions are invalid: `
...
try {
  ...
  try {
    for (const testCase of cases) {
      ...
      for (let question = 0; question < 5; question += 1) {
        if (await completeQuestion(page, testCase)) break;
      }
      ...
      const name = `reading-${testCase.width}x${testCase.height}-${testCase.scene}.png`;
  ...
  console.log(`Reading settlement audit passed: ${results.length} viewports, ${assetCount} assets`);
} finally {
  if (server) {
    server.kill();
    await Promise.race([
