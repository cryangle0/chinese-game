/**
 * Merge customer batch 0722 into existing question-bank.json (batch1 + batch2).
 * Usage:
 *   node tools/bank/import-customer-batch-0722.mjs <xlsx-dir> [out.json] [reading-jumper|writing-treasure]
 */
...
const target = targetGame === 'writing-treasure' ? {
  sourceGame: '挖宝',
  version: 'customer-writing-20260722-batch1+2',
  scenes: new Map([
  ...
  sourceGame: '跳跳乐',
  version: 'customer-reading-20260722-batch1+2',
  scenes: new Map([
...
if (!process.argv[2]) {
  throw new Error('Usage: node import-customer-batch-0722.mjs <xlsx-directory> [out] [game]');
}
...
function stableId(grade, book, stem, options) {
  const digest = crypto.createHash('sha1')
    .update(JSON.stringify([grade, book, stem, options, '0722']))
    .digest('hex')
    .slice(0, 12)
    .toUpperCase();
  return `CB0722_${targetGame === 'writing-treasure' ? 'WT' : 'RJ'}_${grade}_${digest}`;
}
...
for (const file of files) {
  ...
  sheet.eachRow((row, rowNumber) => {
    ...
    imported.push({
      ...
      packId: 'customer-20260722-batch2',
      games: [targetGame],
      scenes: [sceneId],
      grade,
      term: 'ALL',
      ...
      source: `客户题库0722/${gradeLabel}/${book}`,
      enabled: true,
...
if (imported.length < 20) {
  throw new Error(`Only ${imported.length} new customer questions were imported from batch 0722`);
}
