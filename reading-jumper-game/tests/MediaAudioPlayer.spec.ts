...
describe('MediaAudioPlayer', () => {
  ...
  it('stops only the selected looping track during a theme switch', async () => {
    const player = new MediaAudioPlayer();
    await player.play('/mario.mp3', 0.2, true);
    await player.play('/effect.mp3', 0.8);
    player.stop('/mario.mp3');
    expect(FakeAudio.instances.get('/mario.mp3')?.paused).toBe(true);
    expect(FakeAudio.instances.get('/effect.mp3')?.paused).toBe(false);
    player.dispose();
  });
