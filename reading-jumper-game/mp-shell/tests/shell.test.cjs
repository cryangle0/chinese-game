...
test('page opens the configured game and records host results', () => {
  ...
  try {
    ...
    assert.equal(
      ...
      'https://game.xyouxing.com/reading-jumper/question-bank.json',
    );
    assert.equal(url.searchParams.has('trackEndpoint'), false);
...
test('share page turns the navigate query into a native share card', () => {
  ...
  const previousWx = global.wx;
  const app = {
    globalData: { share: { title: '跳跳乐', path: '/pages/game/index', query: 'grade=4' } },
  };
  let definition;
  try {
    ...
    const state = {};
    const page = { ...definition, setData: (value) => Object.assign(state, value) };
    page.onLoad({ title: encodeURIComponent('阅读跳跳乐：40 分') });
    assert.equal(state.title, '阅读跳跳乐：40 分');
    assert.deepEqual(page.onShareAppMessage(), {
      title: '阅读跳跳乐：40 分',
      path: '/pages/game/index?grade=4',
    });
    assert.equal(app.globalData.share.title, '阅读跳跳乐：40 分');
    page.onLoad({});
    assert.equal(state.title, '阅读跳跳乐：40 分');
  } finally {
    global.Page = previousPage;
