...
const port = Number(process.env.PERF_PORT ?? 42982);
...
async function finishStage(page) {
  for (let question = 0; question < 5; question += 1) {
    ...
    for (let attempt = 0; attempt < 10; attempt += 1) {
      if (await page.getAttribute('body', 'data-answer-correct') === 'true') break;
    ...
    try {
      ...
      const diagnostics = await page.evaluate((questionIndex) => ({
        question: questionIndex + 1,
        view: document.body.dataset.gameView ?? 'none',
        gameStage: document.body.dataset.gameStage ?? 'none',
        answerCorrect: document.body.dataset.answerCorrect ?? 'none',
      ...
      throw new Error(
        `benchmark answer did not settle after animation-aware retries: ${JSON.stringify(diagnostics)}`,
        { cause: error },
      );
    }
    await page.waitForFunction((previousQuestionId) => {
      ...
      return document.body.dataset.questionId !== previousQuestionId
        && document.body.dataset.answerCorrect === undefined;
    }, questionId, { timeout: 7000 });
  }
...
async function clickUntilView(page, points, targetView) {
  ...
  throw new Error(`result action did not respond: ${JSON.stringify(diagnostics)}`);
}

async function enterPlay(page) {
  await clickUntilView(
    page,
    [[938, 466], [900, 480], [1020, 480]],
    'play',
  );
}
  ...
  try {
    ...
    while (completedCycles < cycles || (durationMs > 0 && Date.now() - runStartedAt < durationMs)) {
      ...
      const runtimeState = await page.evaluate(() => {
        ...
        const bundleNames = [];
        globalThis.cc?.assetManager?.bundles?.forEach?.((bundle) => {
          bundleNames.push(bundle.name);
        });
        const analyticsRaw = localStorage.getItem('zyb-writing-treasure-analytics-v2') ?? '[]';
        let analyticsEvents = 0;
        try {
          const parsed = JSON.parse(analyticsRaw);
          analyticsEvents = Array.isArray(parsed) ? parsed.length : 0;
        } catch {
          // The byte count still exposes malformed or unexpected persistence.
        }
        return {
          nodeCount,
          domMotionCount: document.querySelectorAll('[data-customer-motion]').length,
          bodyChildCount: document.body.children.length,
          analyticsEvents,
          analyticsBytes: new TextEncoder().encode(analyticsRaw).byteLength,
          bundleNames: bundleNames.sort(),
        };
      });
      heap.push(usedSize);
      heapTimeline.push({
      ...
      if (shouldContinue) {
        if (view === 'stage-result') {
          const finalStage = await page.getAttribute('body', 'data-game-stage') === 'magic';
          await clickUntilView(
            page,
            [[1165, 725], [1165, 740], [1125, 725]],
            finalStage ? 'result' : 'play',
          );
          if (finalStage) {
            await clickUntilView(
              page,
    ...
    if (report.rendering.maxDrawCalls > 40) {
      failures.push(`draw calls max ${report.rendering.maxDrawCalls} exceeds 40`);
    }
    ...
    // The five-question review renders each visible CJK glyph as geometry.
    if (report.rendering.maxResultTriangles > 1200) {
      failures.push(
        `result triangles max ${report.rendering.maxResultTriangles} exceeds 1200`,
      );
    }
