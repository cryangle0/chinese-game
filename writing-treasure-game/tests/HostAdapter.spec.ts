...
describe('HostAdapter launch context', () => {
  ...
  it('defaults production H5 to the trusted diagnostic endpoint', () => {
    Object.defineProperty(globalThis, 'location', {
      configurable: true,
      value: {
        origin: 'https://game.xyouxing.com',
        hostname: 'game.xyouxing.com',
        pathname: '/writing-treasure/index.html',
        protocol: 'https:',
      },
    });
    const context = parseLaunchContext(new URLSearchParams(), 'wechat-mp');
    expect(context.trackEndpoint).toBe(
      'https://agent.onnsa.cn/writing-treasure/api/track',
    );
  });

  it('allows only the trusted cross-origin diagnostic endpoint', () => {
    Object.defineProperty(globalThis, 'location', {
      configurable: true,
      value: {
        origin: 'https://game.xyouxing.com',
    ...
    const trusted = parseLaunchContext(new URLSearchParams(
      'trackEndpoint=https://agent.onnsa.cn/writing-treasure/api/track',
    ), 'wechat-mp');
    const attacker = parseLaunchContext(new URLSearchParams(
      'trackEndpoint=https://attacker.example/track',
    ), 'wechat-mp');
    expect(trusted.trackEndpoint).toBe(
      'https://agent.onnsa.cn/writing-treasure/api/track',
    );
    expect(attacker.trackEndpoint).toBe(
      'https://agent.onnsa.cn/writing-treasure/api/track',
    );
  });
