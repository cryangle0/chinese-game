...
describe('Analytics', () => {
  ...
  it('queues contextualized events and drains them deterministically', () => {
    ...
    analytics.track({ name: 'answer', game: 'writing-treasure', properties: { correct: true } });
  ...
  it('removes a batch only after the endpoint accepts it', async () => {
    ...
    analytics.track({ name: 'app_enter', game: 'writing-treasure' });
  ...
  it('restores persisted events and retains a rejected batch', async () => {
    ...
    first.track({ name: 'queued', game: 'writing-treasure' });
    ...
    analytics.track({ name: 'retry', game: 'writing-treasure' });
    await analytics.flush();
