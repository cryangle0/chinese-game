...
export class QuestionBank {
  ...
  createCursor(
    ...
    if (unused.length >= AppConfig.maxQuestions || unused.length === pool.length) {
      return new QuestionCursor(weightedOrder(unused.length ? unused : pool, random));
    }
    const reused = pool.filter((question) => excludedIds.has(question.id));
    return new QuestionCursor([
      ...weightedOrder(unused, random),
      ...weightedOrder(reused, random),
    ]);
  }
}
